...
export const BookingFlow = React.memo(function BookingFlow({ suite }: BookingFlowProps) {
  ...
  const [datesUnavailable, setDatesUnavailable] = React.useState(false)
  const [submitError, setSubmitError] = React.useState<string | null>(null)

  React.useEffect(() => {
    if (suite.id) {
      getBlockedDates(suite.id)
        .then(setBlockedDates)
        .catch(() => setDatesUnavailable(true))
    }
  }, [suite.id])
          ...
          <>
            {datesUnavailable && (
              <p className="text-red-500 text-sm mb-4">
                Unable to load availability. Please refresh or contact us directly.
              </p>
            )}
            {suite.isHourly ? (
              <HourlySelection

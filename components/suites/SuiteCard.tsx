  ...
  return (
    <div className="relative overflow-hidden rounded-t-2xl h-48 sm:h-64">
      ...
      <div className="absolute top-3 sm:top-4 left-3 sm:left-4" aria-hidden="true">
        <div className="w-8 h-8 sm:w-10 sm:h-10 bg-white/20 backdrop-blur-xl rounded-full flex items-center justify-center shadow-lg shadow-black/10 border border-white/20" data-testid="suite-icon">
...
const SuiteActions = React.memo(function SuiteActions({ suite, onSelect }: { suite: Suite; onSelect: (suiteId: string) => void }) {
  ...
  // onSelect is handled by parent Card now for A11y
    ...
    <div className="space-y-3">
      ...
      <div
        className={cn(
          ...
          "w-full bg-primary/90 hover:bg-primary backdrop-blur-sm text-primary-foreground group-hover:scale-105 transition-all duration-300 text-xs sm:text-sm shadow-lg shadow-primary/20 border border-white/20 rounded-xl hover:shadow-xl",
          !suite.available && "opacity-50"
        )}
      >
        {suite.available

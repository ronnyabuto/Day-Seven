...
export default async function AdminPage() {
    ...
    const totalRevenue = bookings.reduce((sum: number, b: Booking) => sum + (b.total_amount || 0), 0)
    const verifiedBookings = bookings.filter((b: Booking) => b.metadata?.verified_id).length
    const initiatedPayments = bookings.filter((b: Booking) => b.metadata?.mpesa_checkout_id).length

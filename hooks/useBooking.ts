...
export function useBooking() {
  ...
  const updateGuestInfo = useCallback((field: keyof GuestInfo, value: any) => {
  ...
  // Field validation
  const validateField = useCallback((field: keyof GuestInfo, value: any) => {
    const newErrors: Record<string, string> = {}

    // Skip validating file objects or booleans as strings
    if (field === 'idFile' || field === 'agreedToRules') return
  ...
  const isFormValid = useCallback((): boolean => {
    ...
    if (!bookingState.guestInfo.agreedToRules) {
      setBookingState(prev => ({
        ...prev,
        validationErrors: {
          ...prev.validationErrors,
          agreedToRules: "You must agree to the House Rules"
        }
      }))
      return false
    }

    if (!bookingState.guestInfo.idFile) {
      setBookingState(prev => ({
        ...prev,
        validationErrors: {
          ...prev.validationErrors,
          idFile: "Government ID is required for verification"
        }
      }))
      return false
    }

    return Object.values(bookingState.validationErrors).every(err => !err)
  ...
  const isGuestInfoComplete =
    ...
    bookingState.guestInfo.agreedToRules &&
    bookingState.guestInfo.idFile !== null &&
    Object.values(bookingState.validationErrors).every(err => !err)
